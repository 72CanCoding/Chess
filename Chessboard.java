...
public class Chessboard {
	...
	public Field getField(int x, int y) {
		if (hasField(x,y)) return fields[x][y];
		return null;
	}
	...
	void markKnightMoves(int x,int y, int n) {
		if(!hasField(x,y) || n < 0) return;
		fields[x][y].setMarked(true);
	    
	    // Stop recursion if no more moves are left
	    if (n == 0) return;

	    // Recursively mark all possible knight moves
